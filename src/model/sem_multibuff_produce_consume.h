...
class SemMultiBuffProduceConsume {
 public:
  SemMultiBuffProduceConsume(const char* prefix_name, off_t buff_amount,
                             off_t buff_capacity)
      : buff_amount_(buff_amount), buff_capacity_(buff_capacity),
        p_buffs_(NULL), p_sort_buffs_(NULL), p_mutex_(NULL),
        p_empty_(NULL), p_stored_(NULL) {
    char name[ShmBase::ShmConfig::SHM_MAX_NAME_LEN] = {0};
    p_buffs_ = new ShmArray<T>*[buff_amount];
    for (off_t which_buff = 0; which_buff < buff_amount_; ++which_buff) {
      snprintf(name, sizeof(name) - 1, "%s_buff%ld", prefix_name, which_buff);
  ...
  bool Close() {
    ...
    if (true == p_mutex_->IsOpen() && false == p_mutex_->Close()) {
      ret = false;
    }

    if (true == p_empty_->IsOpen() && false == p_empty_->Close()) {
      ret = false;
    }

    if (true == p_stored_->IsOpen() && false == p_stored_->Close()) {
      ret = false;
    }
