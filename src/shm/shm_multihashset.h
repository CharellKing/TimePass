...
#ifndef _SHM_SHM_MULTIHASHSET_H_
#define _SHM_SHM_MULTIHASHSET_H_
...
class ShmMultihashset {
 public:
    explicit ShmMultihashset(const char* name):p_head_(NULL), p_ext_(NULL),
    ...
    bool Insert(const T& data) {
        return ShmHashrbtree<T, Compare, HashFunc, EXTEND>::
                InsertMultiple(p_head_, p_bucket_, p_addr_, data);
    }

    /*删除节点, 返回删除节点的个数*/
    off_t Remove(const T& data) {
        return ShmHashrbtree<T, Compare, HashFunc, EXTEND>::
                RemoveAll(p_head_, p_bucket_, p_addr_, data);
    }
    ...
    RbtreeNode<T>* EqualRange(const T& data) {
        return AT(p_addr_, (ShmRbtree<T, Compare>::
                EqualRange(p_addr_,
    ...
    const off_t Count(const T& data)const {
        return ShmRbtree<T, Compare>::
                Count(p_addr_,
                      p_bucket_[HashFunc(data) & (p_head_->bucket_size - 1)],
                      data);
    }
...
#endif /* _SHM_SHM_MULTIHASHSET_H_ */
